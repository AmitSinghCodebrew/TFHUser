...
//---------------------------SAND'S_HELL_CREATIONS----------------ADD_ONS

extension Date {
    func getChatTimeStamp() -> String {
        if self.isToday() { // For same day timing
            return toString(DateFormat.custom("h:mm a"), isForAPI: false)
        } else if self.isLastWeek() { // For last week timing
            return toString(DateFormat.custom("E"), isForAPI: false)
        } else if self.isSameYearAsDate(Date()) { //For timing lies in same year ongoing
            return toString(DateFormat.custom("d MMM"), isForAPI: false)
        } else { // timing before last year
            return toString(DateFormat.custom("d MMM yyyy"), isForAPI: false)
        }
    }
...
public extension Date {
     ...
     */
    func toString(_ format: DateFormat, timeZone: TimeZone = .local, isForAPI: Bool) -> String
    {
...
//        if isForAPI {
            formatter.locale = Foundation.Locale(identifier: "en_US_POSIX")
//        } else {
//            formatter.locale = L102Language.currentAppleLanguage()?.locale
//        }
