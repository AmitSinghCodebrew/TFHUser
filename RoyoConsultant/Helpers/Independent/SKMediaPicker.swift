...
class SKMediaPicker: NSObject {
    ...
    private func presentDocPicker() {
        DispatchQueue.main.async {
            UIApplication.topVC()?.present(self.documentPicker, animated: true, completion: nil)
        }
    }
...
extension SKMediaPicker: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func cacheFolderPath() -> String? {
        let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).map(\.path).first
        return URL(fileURLWithPath: documentsDirectory ?? "").appendingPathComponent("SKMediaPicker").path
    }
}
