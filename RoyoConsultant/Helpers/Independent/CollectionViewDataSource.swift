...
class CollectionDataSource: NSObject, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    init(_ _items: Array<Any>?, _ _identifier: String?, _ _collectionView: UICollectionView, _ _size: CGSize?, _ _edgeInsets: UIEdgeInsets?, _ _lineSpacing: CGFloat?, _ _interItemSpacing: CGFloat?, _ _direction: UICollectionView.ScrollDirection) {
        super.init()
        items = _items
        ...
        if /collectionView?.collectionViewLayout.isKind(of: ArabicCollectionFlow.self) {
            ...
            if /size?.width == 0 || /identifier == VendorFilterCell.identfier {
                layout.estimatedItemSize = size ?? .zero
            } else {
                layout.itemSize = size ?? .zero
            }
            layout.scrollDirection = _direction
            collectionView?.collectionViewLayout = layout
            collectionView?.semanticContentAttribute = L102Language.isRTL ? .forceRightToLeft : .forceLeftToRight
        }
