...
class GoogleSignIn: NSObject, GIDSignInDelegate {
    ...
    override init() {
        super.init()
        GIDSignIn.sharedInstance()?.clientID = Configuration.getValue(for: .PROJECT_CLIENT_ID)
    ...
    func sign(_ signIn: GIDSignIn!, didSignInFor user: GIDGoogleUser!, withError error: Error!) {
        if user != nil {
            let loginData = GoogleAppleFBUserData.init(user.profile.name, user.userID, user.profile.email, user.profile.imageURL(withDimension: 120), user.authentication.idToken)
            UserPreference.shared.socialLoginData = loginData
...
class FBLogin: NSObject {
    ...
    func login(_ success: SuccessCallBack?) {
        ...
        Profile.current = nil
        LoginManager().logOut()
        LoginManager().logIn(permissions: ["email", "public_profile"], from: UIApplication.topVC()) { (result, err) in
            if err != nil {
    ...
    fileprivate func getEmailNameIdImageFromFB() {
        GraphRequest(graphPath: "/me", parameters: ["fields": "id, name, email, picture.width(480).height(480)"]).start { [weak self] (connection, result, err) -> Void in
            ...
            UserPreference.shared.socialLoginData = fbData
...
@IBDesignable class AppleSignInButton: UIView {
    ...
    private var appleData: GoogleAppleFBUserData? {
        didSet {
            UserPreference.shared.socialLoginData = appleData
        }
    }
