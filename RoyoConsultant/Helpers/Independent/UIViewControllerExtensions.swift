...
extension UIViewController {
    ...
    func presentSafariVC(urlString: String, title: String?) {
        ...
        vc.title = title
        vc.preferredControlTintColor = ColorAsset.appTint.color
        presentVC(vc)
    }
    ...
    func alertBoxOK(title: String?, message: String?, tapped: (() -> Void)?) {
        let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
        alert.addAction(UIAlertAction.init(title: VCLiteral.OK.localized, style: .destructive, handler: { (_) in
            tapped?()
        }))
        present(alert, animated: true, completion: nil)
    }
