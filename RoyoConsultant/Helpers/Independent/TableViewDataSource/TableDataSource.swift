...
struct CollectionSizeProvider {
    var cellSize: CGSize
    var interItemSpacing: CGFloat
    var lineSpacing: CGFloat
    var edgeInsets: UIEdgeInsets
    
    func getHeightOfTableViewCell(for numberOfItems: Int, gridCount: Int) -> CGFloat {
        var numberOfGridRows = round((CGFloat(numberOfItems) / CGFloat(gridCount)) + 0.1)
        if numberOfItems <= gridCount && numberOfItems != 0 {
            numberOfGridRows = 1
        }
        let height = (numberOfGridRows * cellSize.height) + (lineSpacing * (numberOfGridRows - 1.0)) + edgeInsets.top + edgeInsets.bottom
...
class TableDataSource<T : HeaderFooterModelProvider, U: CellModelProvider, Z>: NSObject, UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate {
    ...
    init(_ _type: TableType<T, U, Z>, _ _tableView: UITableView, _ withPullToRefresh: Bool? = false) {
        ...
        case .SingleListing(let _items, let _identifier, let _height, let _leadingSwipe, let _trailingSwipe):
            ...
            items = DefaultHeaderFooterModel<Z>.getSingleListingItems(array: _items, identifer: _identifier, height: _height, leadingSwipe: _leadingSwipe, trailingSwipe: _trailingSwipe) as! [T]
    ...
    public func getSingleLisitngAllItems() -> [Z] {
        let returnArray = (items.first?.items)?.map({($0.property?.model)!}) as? [Z]
        return returnArray ?? []
    }
    
    public func getMultipleSectionItems() -> [T] {
        return items
    }
    ...
    public func updateAndReload(for type: UpdateType<T, U, Z>, _ reloadType: ReloadType) {
        ...
        case .SingleListing(let _items):
            items = DefaultHeaderFooterModel<Z>.getSingleListingItems(array: _items, identifer: identifier ?? "", height: height ?? 0.0, leadingSwipe: leadingSwipe, trailingSwipe: trailingSwipe) as! [T]
        case .MultipleSection(let _items):
            items = _items
        }
